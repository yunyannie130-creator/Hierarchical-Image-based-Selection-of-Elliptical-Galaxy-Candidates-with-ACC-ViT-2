...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        weight = torch.eye(197).repeat(batch_size, 1, 1).to(hidden_states.device)
        eye = torch.eye(197).unsqueeze(0).to(hidden_states.device)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, swap=False):
        ...
        if swap:
            ...
            lam_a = torch.ones(batch_size,device=x.device)
            lam_b = 1 - lam_a
